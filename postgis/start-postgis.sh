...
then
    ...
    echo "postgis defaults setup"
    /home/postgis-defaults.sh

    echo "postgres extensions"
    /home/postgres-extensions.sh

    echo "cartodb postgres extension setup"
    /home/cartodb-setup.sh

    echo "create user requested database"
    createdb -U $POSTGRES_USER -O $POSTGRES_USER -T template_postgis $POSTGRES_DATABASE;
fi
